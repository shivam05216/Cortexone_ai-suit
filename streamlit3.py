...
def prepare_course_data(courses):
    # Convert 'Course Rating' to numeric, coerce errors to NaN
    courses['Course Rating'] = pd.to_numeric(courses['Course Rating'], errors='coerce')
    # Fill missing values in 'Course Rating' with 0
    courses['Course Rating'] = courses['Course Rating'].fillna(0)
    # Drop duplicates and rows with missing values in 'Category'
    courses = courses.dropna(subset=['Category'])
